...
def load_data_delete(args,k,eval_indices,test_indices,train_indices):

    train_data, eval_data, test_data, user_init_entity_set, item_init_entity_set = load_rating_delete(args,k,eval_indices,test_indices,train_indices)
    ...
    return train_data, eval_data, test_data, n_entity, n_relation, user_triple_sets, item_triple_sets, user_init_entity_set


def load_rating_delete(args,k,eval_indices,test_indices,train_indices):
    ...
    return dataset_split_delete(rating_np,k,eval_indices,test_indices,train_indices)


def dataset_split_delete(rating_np,k,eval_indices,test_indices,train_indices):
    ...
    # eval_indices = np.random.choice(n_ratings, size=int(n_ratings * eval_ratio),
    #                                 replace=False)  # 从[0:n_ratings]中选择评价集下标
    ...
    user_init_entity_set, item_init_entity_set = collaboration_propagation(rating_np, train_indices,k)
    ...
    return train_data, eval_data, test_data, user_init_entity_set, item_init_entity_set  # data都是三元组


def collaboration_propagation(rating_np, train_indices,k):  # 三元组和选择的下标
...
def kg_propagation(args, kg, init_entity_set, set_size,
                   is_user):  # (args, kg, user_init_entity_set, args.user_triple_set_size, True)
