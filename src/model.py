...
class CKAN(nn.Module):
    ...
    def forward(
        ...
        user_triple_set: list,#这里都是[[head][relation][tail]]每个里面都有对应层数个数的列表
        item_triple_set: list,#这里都是[[head][relation][tail]]每个里面都有对应层数个数的列表
    ):       
        user_embeddings = []
        
        # [batch_size, triple_set_size, dim]
        user_emb_0 = self.entity_emb(user_triple_set[0][0])#就是最开始的head
        # [batch_size, dim]
    ...
    def _knowledge_attention(self, h_emb, r_emb, t_emb):
        ...
        ############################################################
        last_list = []
        for i in range(len(att_weights_norm)):
            att_to_change = att_weights_norm[i].tolist()
            # print(att_to_change)
            to_change_index = heapq.nlargest(10, range(len(att_to_change)), key=lambda x: att_to_change[x])
            # print(to_change_index)
            for j in range(len(att_to_change)):
                if j not in to_change_index:
                    att_to_change[j] = 0
            # print(att_to_change)
            last_list.append(att_to_change)
        last_tensor = torch.tensor(last_list)
        last_tensor = last_tensor.cuda()
        # print(last_tensor.unsqueeze(-1).size())
        #################################################################


        # [batch_size, triple_set_size, dim]
        emb_i = torch.mul(last_tensor.unsqueeze(-1), t_emb)
        # print(att_weights_norm.unsqueeze(-1).size())
        # [batch_size, dim]
